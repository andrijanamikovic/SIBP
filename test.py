...
from keras.layers import SpatialDropout1D, Bidirectional, Conv1D
...
def create_and_train_model(mode, x_train_pad, y_train_matrix, x_valid_pad, y_valid_matrix,
                            word_index, word_vectorization_matrix, dim_glove, maxlen_):
    model = Sequential()
    model.add(Embedding(len(word_index) + 1,
                        dim_glove,
                        weights=[word_vectorization_matrix],
    ...
    if mode == "LSTM":
        ...
        model.add(Bidirectional(LSTM(dim_glove, dropout=0.3, recurrent_dropout=0.3)))
    elif mode == "CNN":
    ...
    plt.xlabel("Epoch", fontsize=14)
    plt.ylabel("Learning rate", fontsize=14)
    plt.savefig(f'learningrate_modified_exponential{mode}.jpg')
    plt.show()
    ...
    plt.ylabel('Loss', fontsize=14)
    plt.legend()
    plt.savefig(f'logloss{mode}.jpg')
    plt.show()
    y_train_matrix_pred = model.predict(x_train_pad)
    y_valid_matrix_pred = model.predict(x_valid_pad)
    ...
    accuracy_valid = accuracy_score(y_valid_true, y_valid_pred)
    ...
    precision_valid = precision_score(y_valid_true, y_valid_pred, average='weighted')

    recall_train = recall_score(y_train_true, y_train_pred, average='weighted')
    recall_valid = recall_score(y_valid_true, y_valid_pred, average='weighted')

    f1_train = f1_score(y_train_true, y_train_pred, average='weighted')
    f1_valid = f1_score(y_valid_true, y_valid_pred, average='weighted')
    ...
    return model, history, y_valid_true, y_valid_pred


def conf_matrix(y_true, y_pred, n_class, class_names, figsize = (6.25, 5), font_scale = 1, annot_kws_size = 12):
...
lstm_model, lstm_history, lstm_y_valid_true, lstm_y_valid_pred = create_and_train_model('LSTM', x_train_pad, y_train_matrix, x_valid_pad, y_valid_matrix,
                                                  word_index, word_vectorization_matrix, dim_glove, maxlen_)
conf_matrix(lstm_y_valid_true, lstm_y_valid_pred, n_class = 3, class_names = ['EAP', 'HPL', 'MWS'])

bilstm_model, bilstm_history, bilstm_y_valid_true, bulstm_y_valid_pred = create_and_train_model('BiLSTM', x_train_pad, y_train_matrix, x_valid_pad, y_valid_matrix,
                                                  word_index, word_vectorization_matrix, dim_glove, maxlen_)
conf_matrix(bilstm_y_valid_true, bulstm_y_valid_pred, n_class = 3, class_names = ['EAP', 'HPL', 'MWS'])


cnn_model, cnn_history, cnn_y_valid_true, cnn_y_valid_pred = create_and_train_model('CNN', x_train_pad, y_train_matrix, x_valid_pad, y_valid_matrix,
                                                  word_index, word_vectorization_matrix, dim_glove, maxlen_)
conf_matrix(cnn_y_valid_true, cnn_y_valid_pred, n_class = 3, class_names = ['EAP', 'HPL', 'MWS'])